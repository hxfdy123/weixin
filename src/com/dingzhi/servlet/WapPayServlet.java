...
public class WapPayServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String ordernum = DateUtil.getCurrentDateStr();//生成订单号 
		   ...
		   map.put("body", "yudingxingcheng");  //商品描述
		   map.put("total_fee", 100);  //支付金额（精确到分）
		   map.put("spbill_create_ip", getRemortIP(req));  //用户IP
		  // String sign = PayCommonUtil.createSign("UTF-8", packageParams, key);
		   ...
		   String mweb_url=getElementValue(in,"mweb_url");//支付跳转，唤起微信客户端
		   
		  // mweb_url+="&redirect_url="+URLEncoder.encode("https://dzpay.zhenqingbus.com","GBK");
		   mweb_url+="&redirect_url="+URLEncoder.encode("http://custombus.zhenqingbus.com","GBK");//支付后跳转页面
